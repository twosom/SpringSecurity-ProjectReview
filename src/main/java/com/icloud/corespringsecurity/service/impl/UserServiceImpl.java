...
public class UserServiceImpl implements UserService {
    ...
    public AccountDto getAccount(Long id) {
        ...
        if (account.getRoleList() != null) {
            List<String> roleList = account.getRoleList().stream().map(Role::getRoleName).collect(Collectors.toList());
    ...
    public void modifyAccount(AccountDto accountDto) {
        ...
        if (accountDto.getRoleList() != null) {
            ...
            accountDto.getRoleList().forEach(role -> {
                roleList.add(roleRepository.findByRoleName(role));
            });

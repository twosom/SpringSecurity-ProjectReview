...
public class UserServiceImpl implements UserService {
    ...
    public AccountDto getAccount(Long id) {
        ...
        if (account.getRoleList() != null) {
            List<String> roleList = account.getRoleList().stream().map(Role::getRoleName)
    ...
    public void modifyAccount(AccountDto accountDto) {
        ...
        if (accountDto.getRoleList() != null) {
            List<Role> roleList = accountDto.getRoleList().stream().map(roleRepository::findByRoleName)
                    .collect(Collectors.toList());

...
public class ResourcesServiceImpl implements ResourcesService {
    ...
    public ResourcesDto getResources(Long id) {
        ...
        if (resources.getRole() != null) {
            resourcesDto.setRoleName(resources.getRole().getRoleName());
        }
    ...
    public LinkedHashMap<RequestMatcher, List<ConfigAttribute>> getResourcesMap() {
        ...
        allResources.forEach(resources -> {
            String roleName = resources.getRole().getRoleName();
            List<ConfigAttribute> attribute = Arrays.asList(new SecurityConfig(roleName));

            result.put(new AntPathRequestMatcher(resources.getResourceName(), resources.getHttpMethod().name()), attribute);
        });
